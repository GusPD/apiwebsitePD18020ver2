...
function listarProductos(productos) {
	...
	for(nfila=0;nfila<num;nfila++) {
		...
		eliminar[nfila].innerHTML="<button>Eliminar</button>";
		eliminar[nfila].firstChild.setAttribute("onclick","eliminar('"+productos[nfila].id+"');");
	}
} 
function obtenerProductos() {
	  fetch('https://retoolapi.dev/4AN02n/productos')
            .then(res=>res.json())
            .then(data=>{
				productos=data;
				productos.forEach(
...
function agregar(){
	var producto={
		...
		price:document.getElementById('priceL').value,
	...
	orden*=0;
	setTimeout(obtenerProductos, 1000);
}

function eliminar(id){
	var opt=confirm("Desea eliminar el registro: "+id);
	if(opt==true){
		fetch("https://retoolapi.dev/4AN02n/productos/"+id,
		{ method:"DELETE"})
		.then(response=>response.json())
		.then(data=>data);
